...
function updateModalUI() {
    // Note: The HTML quantity display is a span with innerText
...
function populateBundleSelectors(id) {
    ...
    for (let i = 1; i <= 3; i++) {
        ...
        selectElement.className = 'choice-select form-control'; // Added form-control class
...
function openDetails(id, name, desc, price, img, isSweet) { 
    ...
    if (qtySelector) {
        if (selectedItem.isBundleDeal) {
            currentQuantity = 1; 
            updateModalUI();
            // This class hides the buttons if the deal must be fixed at Qty 1
            qtySelector.classList.remove('fixed-qty'); // Removed 'add' to allow changes
        } else {
...
/**
 * Fix implemented here: Allows quantity change for the entire bundle/item.
 * @param {number} change - +1 or -1.
 */
function changeQty(change) {
    // No longer checks for selectedItem.isBundleDeal and returns.
    // This allows the user to buy multiple bundles (e.g., two 3-Packs).
...
function addToCart() {
    ...
    // Bundle items should always be treated as a new unique line item due to choices/requests
...
function renderCartItems() {
    ...
    if (cartItems.length === 0) {
        ...
        cartItems.forEach((item, index) => {
            ...
            // Note: Truncation logic is simple here for display purposes
...
function viewCart() {
    ...
    // Using class manipulation is generally non-Bootstrap-specific
...
function startCheckout() {
    ...
    // In a real application, you'd check a session variable or cookie here
